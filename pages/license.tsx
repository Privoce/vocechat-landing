...
const Index = (props: Props) => {
    ...
    const handleSubmit = async (evt: FormEvent<HTMLFormElement>) => {
        ...
        try {
            setLicense("")
            setGenerating(true)
            const resp = await axios.post(`https://vera.nicegoodthings.com/vocechat/landing/license`, { secret, data: body })
    ...
    return (
        <main className="w-screen h-screen bg-gray-50 flex flex-col justify-center items-center text-2xl">
            <h1 className="text-xl md:text-4xl font-bold mb-2 md:mb-10 flex flex-col items-center gap-2">
                <span>VoceChat License Generator</span>
            ...
            <form action="" onSubmit={handleSubmit} className="flex flex-col justify-start gap-2">
                ...
                <button type="submit" disabled={generating} className="p-1 md:p-2 rounded-full bg-success-700 text-white mt-2 md:mt-6 disabled:bg-gray-400 disabled:cursor-wait">{secret ? generating ? `Generating` : `Generate` : "Set Secret"}</button>
            </form>
            {secret && license && <>
