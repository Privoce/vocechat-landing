...
export default function License() {
  ...
  const handleSubmit = async (evt: FormEvent<HTMLFormElement>) => {
    ...
    const secretVal = (formEle as any).secret?.value;
    if (secretVal) {
      try {
        setPairing(true);
    ...
    <main className="w-screen h-screen bg-gray-50 flex flex-col justify-center items-center text-2xl">
      <h1 className="text-xl md:text-4xl font-bold mb-2 md:mb-10 flex flex-col items-center gap-2">
        <span className="relative">
          ...
          {name && (
            <em className="absolute right-0 -top-4 rotate-6 text-primary-500 text-xs  not-italic">
              {name}
            </em>
          )}
        </span>
        {secret ? (
          <button
            onClick={handleReset}
            className="bg-red-500 text-white px-4 py-1 rounded-full text-xs"
          >
            Reset Secret
          </button>
        ) : null}
      ...
      <form action="" onSubmit={handleSubmit} className="flex flex-col justify-start gap-2">
        {secret ? (
          ...
          <input
            ...
            type="text"
            className={inputClass}
            placeholder="Input Secret Please"
          ></input>
        )}
        <button
          type="submit"
          disabled={generating || pairing}
          className="p-1 md:p-2 rounded-full bg-success-700 text-white mt-2 md:mt-6 disabled:bg-gray-400 disabled:cursor-wait"
        >
      ...
      {secret && license && (
        <>
          ...
          <CopyToClipboard text={license} onCopy={handleCopy}>
            <button className="text-lg text-white bg-primary-600 px-4 py-1 mt-4 rounded-full">
              {copied ? "Copied" : "Copy"}
            </button>
          </CopyToClipboard>
        </>
