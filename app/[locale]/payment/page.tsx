  ...
  return (
    <main className="w-screen h-screen bg-gray-50 flex flex-col justify-center items-center text-2xl">
      ...
      <span className="text-xs text-gray-500 mb-5">
        Please input your host address,e.g.
        <code className="bg-gray-200 p-1">vocechat.yourdoamin.com</code>
      </span>
      <form action="" onSubmit={handleSubmit} className="flex flex-col justify-start gap-2">
        {sessionId ? (
          <input
            required
            className={"text-lg p-2 border border-solid border-gray-100 rounded"}
            name="domain"
        ...
        <button
          disabled={!sessionId}
          type="submit"
          className="p-2 rounded-full bg-success-700 text-white mt-6"
        >
          Generate License
        </button>
      </form>
