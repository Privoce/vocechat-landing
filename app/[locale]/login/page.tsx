...
export default function Login() {
  ...
  const link = decodeURIComponent(searchParams.get("s") ?? "");
        ...
        {link && (
          <li className="text-gray-600 text-center w-full flex flex-col gap-2 mt-2">
            <i className="text-gray-400 not-italic text-xs break-words">
              👇 App not showing? You may copy the following link and paste it into VoceChat App.
            </i>
            <textarea
              onClick={handleTextareaClick}
              readOnly
              value={link}
              className="bg-gray-200 font-bold p-2 rounded-md break-all"
              spellCheck={false}
            ></textarea>
